...
> 本页只放 **可引用的公开文档结论** 与本仓 **Doubao Work 观测** 的并排。  
> 不编造对方内部细节。Doubao 列均标注观测日 / `IMAGE_VERSION`。  
> Doubao 侧：捕获 **2026-09-24**，`IMAGE_VERSION=1.14.10`。
...
| 网络模式 | 受控出口代理 + 域名分流样例（见 [network.md](network.md)）；非「随意公网」 | `PUBLIC` 或 `VPC`（[VPC guide](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agentcore-vpc.html)） | 托管沙箱出站由平台策略约束；自托管则出站留在你的环境 |
...
- 读 Doubao 列时回到对应 `docs/*.md` 看路径级证据。  
