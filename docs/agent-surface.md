...
| CC 面 | 本机 10000 端口的 `/cc/` | 名称带 `CC` 的一组接口 | 跑命令、读写文件、搜索文件等 | 需要平台签名 |
...
除了挂给模型的 browser 服务，应用里还有一套 FastMCP 工具，名称类似「Sandbox MCP Tools」，当次版本号为 2.14.7。  
...
审查失败一类文案，只出现在会话产物里；在容器文件系统里搜不到对应策略文件。  
用多种「历史上像会挂」的命令写法复测时，当次反而都通过了。  
因此更稳妥的写法是：**未测到容器内的策略落点**；具体拦在哪一层、按什么规则拦，仍属推断。
